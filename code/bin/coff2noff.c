...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < numsections; i++) {
		printf("\t\"%.8s\", filepos 0x%lx, mempos 0x%lx, size 0x%lx, flags 0x%lx\n", sections[i].s_name, sections[i].s_scnptr, sections[i].s_paddr,
		       sections[i].s_size, sections[i].s_flags);
		if (sections[i].s_size == 0) {
		...
		} else if (!strncmp(sections[i].s_name, ".data", 8) || !strcmp(sections[i].s_name, ".rdata")) {
			...
			free(buffer);
			inNoffFile += sections[i].s_size;
		} else if (!strncmp(sections[i].s_name, ".bss", 8) || !strcmp(sections[i].s_name, ".sbss")) {
			/* need to check if we have both .bss and .sbss -- make sure they
			 * are contiguous
			 */
			if (noffH.uninitData.size != 0) {
				if (sections[i].s_paddr == (noffH.uninitData.virtualAddr + noffH.uninitData.size)) {
